...
class Metric:
    ...
    def _prepare_label_for_retrieval(eval_data):
        ...
        with open(eval_data) as f:
            for i, line in tqdm(enumerate(f), total=total1, desc="Reading train"):
                item = json.loads(line)
                if "query_id" in item:
                ...
                score = [1] * len(item["pos_index"])
                labels[query_id] = (item["pos_index"], score)
        return labels
    
    @staticmethod
    def _prepare_pred_for_retrieval(preds:List[RerankResult]) -> List[RerankResult]:
        valid_preds = [x for x in preds if int(x.doc_id) > -1]
        return valid_preds

    @staticmethod
    def rouge(eval_data=None, **kwds):
        if eval_data is not None:
            data_labels = Metric._prepare_label_for_generation(eval_data)
        
        rouge = Rouge()

        def compute_metric(results, labels=None, **kwargs):
            if labels is None:
                labels = data_labels

            if len(results) != len(labels):
                logger.warning(f"There are {len(results)} queries in predictions while {len(labels)} queries in labels!")

            # reorder labels according to index
            labels = [labels[key] for key in results.keys()]

            preds = normalize_text(list(results.values()))
